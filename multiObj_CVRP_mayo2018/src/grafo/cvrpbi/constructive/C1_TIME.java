 ...
 */
public class C1_TIME implements Constructive<WCPInstance, WCPSolution> {
	...
	public WCPSolution constructSolution(WCPInstance instance) {
		...
		while (!cl.isEmpty()) {
			if (insertedByRegret < nodesPerRegret) {
				...
				//System.out.println("distancia actual = "+ sol.getRoute(maxRegret.r).getTime()+"; Incremento = "+insertCosts[selected].time[maxRegret.r]);
				...
				//System.out.println("nueva distancia ="+sol.getRoute(maxRegret.r).getTime());
