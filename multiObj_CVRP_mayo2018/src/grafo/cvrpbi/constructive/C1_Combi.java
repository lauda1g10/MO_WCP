 ...
 */
public class C1_Combi implements Constructive<WCPInstance, WCPSolution> {
		...
		while (!cl.isEmpty()) {
			if (insertedByRegret < nodesPerRegret) {
				...
				sol.addNode(selected, maxRegret.r, selectedInsertCosts[selected].pos[maxRegret.r],
						selectedInsertCosts[selected].dist[maxRegret.r],selectedInsertCosts[selected].time[maxRegret.r]);
				...
				sol.addNode(selected, minRoute, selectedInsertCosts[selected].pos[minRoute],
						selectedInsertCosts[selected].dist[minRoute],selectedInsertCosts[selected].time[minRoute]);
